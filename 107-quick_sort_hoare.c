...
/**
 * quicksort_recursive - Implements the recursive quicksort algorithm.
...
void quicksort_recursive(int *array, ssize_t start, ssize_t end, int array_size)
{
	...
	if (start < end)
	{
		...
		/* Sort the left side */
		quicksort_recursive(array, start, partition_point - 1, array_size);
		/* Sort the right side */
		quicksort_recursive(array, partition_point, end, array_size);
	}
}
...
void quick_sort_hoare(int *array, size_t array_size)
{
	...
	/* Start the quicksort */
	quicksort_recursive(array, 0, array_size - 1, array_size);
}
